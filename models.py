    ...
    def validate_publication_date(cls, values):
        ...
        if pub_day:
            try:
                ...
                raise ValueError(f"Validation error: {value_error}")
        else:
            if pub_month < 1 or pub_month > 12:
                raise ValueError("Validation error: month out of range")
            if pub_year < 1 or pub_year > 9999:
                raise ValueError("Validation error: year out of range")
        return values
